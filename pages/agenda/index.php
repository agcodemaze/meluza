...
if (isset($_GET['data_inicio']) && isset($_GET['data_fim'])) {
    try {
        ...
        $dataFim = (new DateTime())->modify('+7 days')->setTime(23, 59, 59)->format('Y-m-d H:i:s');
    }
} else {
                                        ...
                                        <div class="px-2">
                                            <?php foreach ($faxinas as $item): ?>
                                                <?php
                                                    ...
                                                    $dataOriginalSemana = $item["FXA_DTDATA"];
                                                    $data = new DateTime($dataOriginalSemana);
...
<script>  
  ...
  function initFlatpickr(mode = 'single') {
    ...
    flatpickrInstance = flatpickr("#dataHora", {
      enableTime: true,
      dateFormat: "d/m/Y H:i",
      time_24hr: true,
      locale: "pt",
      ...
      defaultMinute: 0,
      minuteIncrement: 5,
      mode: mode // 'single' ou 'multiple'
    });
  }

  // Inicializa flatpickr default em modo single
  ...
  async function buscarCoordenadas(endereco) {
    ...
    try {
      const resposta = await fetch(url, {
        headers: { 'User-Agent': 'Codemaze/1.0 (suporte@codemaze.com.br)' }
      });
  ...
  faxinaItens.forEach(item => {
    item.addEventListener('click', function () {
      // EDIT MODE: flatpickr single date
      initFlatpickr('single');
  ...
  window.abrirModalNovo = function () {
    // NEW MODE: flatpickr multiple dates
    initFlatpickr('multiple');
